...
if [[ ! -f "\${BOTTLE_PATH}/drive_c/Program Files/Adobe/Adobe Creative Cloud/ACC/Creative Cloud.exe" ]]; then
    ...
    wget -q -O "\${CC_INSTALLER}" \
        "https://ccmdls.adobe.com/AdobeProducts/KCCC/1/osx10/CreativeCloudSetup.exe" || \
    wget -q -O "\${CC_INSTALLER}" \
        "https://prod-rel-ffc-ccm.oobesaas.adobe.com/adobe-ffc-external/core/v1/wam?payload=ssai&appName=creativeclouddesktop" 2>/dev/null || true
