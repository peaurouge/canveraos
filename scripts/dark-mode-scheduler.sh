...
apply_dark_mode() {
    ...
    # Wallpaper
    plasma-apply-wallpaperimage /usr/share/canvera/wallpapers/canvera-dark.png 2>/dev/null || true
    # Latte Dock opacity
    qdbus org.kde.lattedock /Latte setProperty panelTransparency 80 2>/dev/null || true
    # Save state
...
apply_light_mode() {
    ...
    # Wallpaper
    plasma-apply-wallpaperimage /usr/share/canvera/wallpapers/canvera-light.png 2>/dev/null || true
    # Latte Dock opacity
    qdbus org.kde.lattedock /Latte setProperty panelTransparency 70 2>/dev/null || true
    # Save state
...
enabled=${enabled_str}
dark_start=${dark_new}
light_start=${light_new}
current_mode=$(grep "current_mode" "${CONFIG_FILE}" | cut -d= -f2 | tr -d ' ')
CONF
